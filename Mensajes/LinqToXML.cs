...
namespace Mensajes
{
    ...
    public static class LinqToXml
    {
        ...
        [CLSCompliant(false)]
        public static Entidades.Mensaje LeerMensaje(string codigoMensaje)
        {
           ...
           System.Collections.Generic.IEnumerable<System.Xml.Linq.XElement> elementoXML = from el in documentoXML.Root.Elements("mensaje")
                          where el.Element("codigo") != null && el.Element("codigo").Value == codigoMensaje
                          select el;

           if (elementoXML.Count() != 0)
           {
               return new Entidades.Mensaje() 
               { 
        ...
        /// <returns>Objeto con los datos del registro tomados del archivo XML</returns>
        [CLSCompliant(false)]
        public static Entidades.EtiquetaControles LeerEtiquetaControles(string codigoEtiqueta)
        {
            string rutaArchivo = string.Empty;
            System.Xml.Linq.XDocument documentoXML = null;
            ...
            if (elementoXML.Count() != 0)
            {
                return new Entidades.EtiquetaControles()
                {
                    Codigo = elementoXML.ElementAt(0).Element("Codigo").Value,
